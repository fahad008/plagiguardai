<?php $this->load->view('header'); ?>
	...
	<script>
		...
		var bulkDropzone = new Dropzone("#kt_modal_bulk_attachments", {
		    ...
		    init: function () {
		        ...
		        // When each file is successfully uploaded
		        ...
		        // When all files in queue are finished
		        dz.on("queuecomplete", function() {
		            // console.log("All files uploaded successfully");
		            // Optional: trigger additional action
	...
	//     autoProcessQueue: false,   // ⭐ IMPORTANT (disable instant upload)
	//     uploadMultiple: true,
	//     parallelUploads: 5,
	//     maxFiles: 5,
	//     maxFilesize: 5,
	...
	//         // ⭐ Upload only when form submitted
	//         document.querySelector("#kt_bulk_upload_form").addEventListener("submit", function (e) {
	//             e.preventDefault();

	//             if (dz.getQueuedFiles().length > 0) {
	//                 dz.processQueue();   // manually start upload
	//             } else {
	//                 this.submit();       // no files → normal submit
	//             }
	//         });
	...
	//                 files.forEach(file => dz.removeFile(file));


	//                 showInfo('#textarea-error', response.message);
	//                 scrollToTextarea('#textarea-error', 100);

	//                 // ⭐ after upload success submit form
	//                 document.querySelector("#kt_bulk_upload_form").submit();

	//             } else {
	//                 files.forEach(file => dz.emit("error", file, response.message));
